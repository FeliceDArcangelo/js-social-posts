...
sharePosts(posts, eleContainer);



function sharePosts(arrPosts, eleContainer) {
	eleContainer.innerHTML = arrPosts.reduce((postsHtml, objPost) => postsHtml + postsHtml(objPost), '');
	const listLikeButtons = eleContainer.querySelectorAll('.like-button');
	listLikeButtons.forEach(likeButton => likeButton.addEventListener('click', btnButton));
}

function postsHtml(objPost) {
	return `
		<div class="post">
...
function btnButton() {
	...
	if (isLiked.includes(postId)) {
		const indexPost = isLiked.indexOf(postId);
		isLiked.splice(indexPost, 1);
		variation = -1;
	} else {
		isLiked.push(postId);
		variation = 1;
...
function manageClassLike(objPost) {
	return isLiked.includes(objPost.id) ? 'like-button--liked' : '';
}

function formatIsoToItalianDate(isoData) {
	return isoData.split('-').reverse().join('/');
}
